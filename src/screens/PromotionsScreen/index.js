...
import { View, Image, TouchableOpacity, ScrollView, ImageBackground, Text, FlatList, Platform, PermissionsAndroid } from 'react-native';
...
const Login = props => {
    ...
    const footer = () => {
        return (
            <View style={{
                ...
                borderWidth: 0.2 * vh,
                // height:40*vh,
                alignItems: 'center', marginBottom: 2 * vh
            }}>
                <View style={{ flexDirection: 'row', alignItems: 'flex-end' }}>
                    {user?.user?.type == 0 && <Button title="UPLOAD" style={{ width: 25 * vw }}
                    />}
                    <TouchableOpacity style={{ marginLeft: 10 * vw }}
                        onPress={() => showUpload()}>
        ...
        dispatch(
            ...
            .then(res => {
                ...
                if (res?.payload?.message == 'Success') {
                    ...
                    if (user?.user?.type == 0) {
                        photos = res?.payload?.data.filter((item) =>

                            item?.user_id == user?.user?._id
                        )
                    }
                    setData(photos)
                }
    ...
    return (
        ...
        <ImageBackground source={images.bg}
            ...
            />
            {user?.user?.type == 0 && <View style={{ marginTop: 3 * vh, flexDirection: 'row' }}>
